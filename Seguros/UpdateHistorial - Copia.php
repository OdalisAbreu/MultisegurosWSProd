<?
	...
	function IfMontoCostoTarifasHistory($veh_tipo,$vigencia){
	   $queryT=mysql_query("SELECT id,veh_tipo,3meses,6meses,12meses 
	   FROM seguro_costos_backup WHERE veh_tipo ='".$veh_tipo."'  LIMIT 1");
  ...
  if($_GET['consul']=='1'){
	...
	while($row=mysql_fetch_array($query)){
	 ...
	 if($row['tipo']=='serv'){
		...
		$Veh 		= explode("|", Datos($row['id_trans']));
		$costoseg 	= IfMontoCostoTarifasHistory($veh_tipo,$vigencia);
		echo "<tr>
				<td>VIGENCIA</td>
		...
		mysql_query("UPDATE seguro_trans_history SET id_aseg ='".$id_aseg."',
		costo = '".$costoseg."' WHERE id='".$row['id']."' LIMIT 1");
