...
function FechaHoraNew($id){
 ...
 $html .='No. Poliza,Nombres,Apellidos,Cedula,Ciudad,Telefono,Marca,Modelo,Year,Chassis,Placa,Fecha Emision,Agencia,Inicio Vigencia,Fin Vigencia,Prima';
	...
	$SUMtotal +=$precio;
$html .='
	'.$t.','.$prefi.','.$cliente[0].','.$cliente[1].','.Cedula($cliente[2]).','.Ciudad($cliente[4]).','.Telefono($cliente[5]).','.$marca.','.$modelo.','.$veh[3].','.$veh[5].','.$veh[4].','.FechaHoraNew($u['fecha']).','.$u['id'].','.Fecha($u['fecha_inicio']).','.Fecha($u['fecha_fin']).',$'.formatDinero($precio).'';
...
';
	...
	$sfile	= "CSV/DOM/MS_RDV_$fDesde.csv"; // Ruta del archivo a generar 
